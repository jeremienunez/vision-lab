...
use perception_app::{
    ...
    ExportYoloAnnotationsUseCase, SampleDraft, SampleRepository, TaskType, UseCaseError,
};
...
#[tokio::test]
async fn export_yolo_annotations_returns_classes_and_one_label_file_per_sample() {
    ...
    let sample_b = samples
        .create(sample_fixture(dataset.id, "b-cup.jpg"))
        .await
        .expect("sample is created");
    let sample_a = samples
        .create(sample_fixture(dataset.id, "a-book.jpg"))
        .await
        .expect("sample is created");
    annotations
        .create(annotation_fixture(dataset.id, sample_b.id, 0, "cup"))
        .await
        .expect("annotation is created");
    annotations
        .create(annotation_fixture(dataset.id, sample_a.id, 1, "book"))
        .await
        .expect("annotation is created");

    let export = ExportYoloAnnotationsUseCase::new(&datasets, &samples, &annotations)
        .execute(dataset.id)
        .await
        .expect("YOLO export is created");

    assert_eq!(export.dataset_id, dataset.id);
    assert_eq!(export.classes_txt, "cup\nbook\n");
    assert_eq!(export.files.len(), 2);
    assert_eq!(export.files[0].path, "labels/a-book.txt");
    assert_eq!(
        export.files[0].content,
        "1 0.250000 0.400000 0.300000 0.400000\n"
    );
    assert_eq!(export.files[1].path, "labels/b-cup.txt");
    assert_eq!(
        export.files[1].content,
        "0 0.250000 0.400000 0.300000 0.400000\n"
    );
}

#[tokio::test]
async fn export_yolo_annotations_rejects_missing_dataset() {
    let datasets = InMemoryDatasetRepository::default();
    let samples = InMemorySampleRepository::default();
    let annotations = InMemoryAnnotationRepository::default();

    let result = ExportYoloAnnotationsUseCase::new(&datasets, &samples, &annotations)
